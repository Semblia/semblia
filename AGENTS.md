...
- For every new codebase task, Codex and Claude must work through a fresh branch and pull request. Do not make new task changes directly on `main`. Open the PR before considering the task complete, and prefer leaving PR-bot/reviewer follow-up to a scheduled later pass with a minimum 5-hour delay when usage limits or task length make same-session cleanup impractical.
