...
export class ImportProcessingRuntime {
  ...
  private async processPublicImport(job: WorkerJob, config: WorkerConfig) {
    ...
    const connection = job.connectionId
      ? await this.context.getConnection(job.projectId, job.connectionId)
      : null;
  ...
  private async runConnectedPages(input: {
    ...
    for (
      ...
      if (state.cursor && seenCursors.has(state.cursor))
      ...
      const counts = await this.persistCandidates(
        input.job,
        candidates,
      ...
      if (
        truncated ||
        !page.nextCursor ||
        state.total >= MAX_CONNECTED_IMPORT_ITEMS
      )
        return state;
      await this.context.prisma.client.importJob.update({
  ...
  private async recordFailedItem(
    ...
    if (existing) return;
    try {
      await this.context.prisma.client.importItem.create({
        data: {
          jobId: job.id,
          rowIndex,
          result: "FAILED",
          sourceUrl,
          externalIdHash: candidateIdentityHash(job.sourceKey, { externalId }),
