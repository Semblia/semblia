...
import type { AllowedOutboundWebhookTarget } from "./outbound-webhook-url-safety.js";
...
export function requestPinnedTarget(
...
function targetLookup(input: { target: AllowedOutboundWebhookTarget }) {
...
function writeRequestBody(input: {
  ...
  input.request.destroy(new Error("Unsupported webhook request body type"));
}
