...
const envSchema = z
  .object({
    ...
    /**
     * Project signing secret used by forms when it proxies browser
     * submissions to api. The public API expects X-Semblia-Signature over
    ...
    /**
     * Temporary bridge until api exposes host-based runtime resolution. The
     * current api route still validates `projectId` as a required query.
     */
