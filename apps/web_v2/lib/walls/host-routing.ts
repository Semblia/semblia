...
export function normalizeWallHostname(value: string | null | undefined): string | null {
  if (!value) return null;
  const candidate = value.trim().toLowerCase();
  const port = candidate.match(/:(\d+)$/)?.[1];
  if (port && (Number(port) > 65535 || Number(port) < 1)) return null;
  const hostname = (port ? candidate.slice(0, -(port.length + 1)) : candidate).replace(/\.+$/, "");
...
export function projectWallHostname(host: string | null | undefined): string | null {
