...
function prisma(
  ...
  allHosts: ReturnType<typeof host>[] = [],
) {
  const client = {
    project: { findMany: vi.fn().mockResolvedValue(projects) },
    publicSurfaceHost: {
      findMany: vi.fn().mockResolvedValue(allHosts),
      create: vi.fn(),
      update: vi.fn(),
    },
...
describe("public hosting backfill", () => {
  ...
  it("repairs resource/default/managed verification and preserves legacy rows without deletion", async () => {
    const legacy = host({
      id: "legacy",
      hostname: "https://Acme.Testimonials.Semblia.com.:443",
      isDefault: true,
      verifiedAt: null,
    });
    const current = host({
    ...
    const db = prisma(
      [project({ publicSurfaceHosts: [legacy, current] })],
      [legacy, current],
    );
    ...
    expect(db.publicSurfaceHost.update).toHaveBeenCalledWith(
  ...
  it("leaves invalid labels, hostname collisions, and unverified external hosts manual and untouched", async () => {
    ...
    const db = prisma(
      [project({ slug: "www", publicSurfaceHosts: [external] })],
      [external],
    );
    const summary = await backfillPublicHosting(db as never, options(true));
    expect(summary).toMatchObject({
      unverifiedExternalHosts: 1,
  ...
  it("normalizes safely, but treats normalized duplicates and retired reservations as manual", async () => {
    ...
    expect(safeDb.publicSurfaceHost.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: "host_1" },
        data: { hostname: "acme.forms.semblia.com" },
      }),
    );
    ...
    const conflictDb = prisma(
      [project({ publicSurfaceHosts: [safe, duplicate] })],
      [safe, duplicate],
    );
    await expect(
      backfillPublicHosting(conflictDb as never, options(true)),
    ).resolves.toMatchObject({
      normalizationConflicts: expect.any(Number),
      manualResolutionRequired: 1,
    });
    expect(conflictDb.$transaction).not.toHaveBeenCalled();

    const retired = host({ status: "DISABLED", retiredAt: date });
    const retiredDb = prisma(
      [project({ publicSurfaceHosts: [retired] })],
      [retired],
    );
    await expect(
      backfillPublicHosting(retiredDb as never, options(true)),
    ).resolves.toMatchObject({ manualResolutionRequired: 1 });
    expect(retiredDb.publicSurfaceHost.create).not.toHaveBeenCalled();

    const foreignTombstone = host({
      id: "foreign",
      projectId: null,
    ...
    await expect(
      backfillPublicHosting(crossDb as never, options(true)),
    ).resolves.toMatchObject({ manualResolutionRequired: 1 });
  ...
  it("requires manual review for mismatched project resources and keeps logs identifier-only", async () => {
    const broken = host({
      resourceId: "other_project",
      hostname: "acme.forms.semblia.com",
    });
    const input = options(true);
    const db = prisma([project({ publicSurfaceHosts: [broken] })], [broken]);
    const summary = await backfillPublicHosting(db as never, input);
    expect(summary).toMatchObject({
      inconsistentResources: 1,
      manualResolutionRequired: 1,
    });
    expect(input.log).toHaveBeenCalled();
    expect(input.log.mock.calls.flat().join(" ")).not.toContain(
  ...
  it("repairs an eligible primary deterministically and treats no eligible wall as informational", async () => {
    ...
    expect(db.widget.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    await expect(
      backfillPublicHosting(noWall as never, options()),
    ).resolves.toMatchObject({ noEligibleWallProjects: 1, changed: 0 });
  });
  ...
  it("clears duplicate and ineligible primaries, and retries a hostname P2002 only as a whole transaction", async () => {
    ...
    expect(primaryDb.widget.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    raceDb.publicSurfaceHost.create.mockRejectedValue({
      code: "P2002",
    ...
    await expect(
      backfillPublicHosting(unrelatedDb as never, options(true)),
    ...
    const client = {
      ...
      publicSurfaceHost: {
        ...
        create: vi.fn(async () => {
          ...
          throw { code: "P2002", meta: { target: ["hostname"] } };
        }),
        update: vi.fn(),
      },
      widget: { updateMany: vi.fn() },
  ...
  it("does not count or log applied work from a transaction rolled back by a later hostname collision", async () => {
    const db = prisma([project()]);
    const input = options(true);
    db.publicSurfaceHost.create
      .mockResolvedValueOnce(undefined)
      .mockRejectedValue({ code: "P2002", meta: { target: ["hostname"] } });
    const summary = await backfillPublicHosting(db as never, input);
  ...
  it("parses only explicit apply or dry-run flags", () => {
    ...
    expect(parsePublicHostingBackfillArgs(["--", "--dry-run"])).toEqual({
      apply: false,
    });
    expect(parsePublicHostingBackfillArgs(["--dry-run"])).toEqual({
      apply: false,
    });
    expect(parsePublicHostingBackfillArgs(["--apply"])).toEqual({
      apply: true,
    });
    expect(() =>
      parsePublicHostingBackfillArgs(["--apply", "--dry-run"]),
    ).toThrow("Usage");
