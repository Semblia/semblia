...
only as implementation archaeology when validating why a current `api`
...
### `apps/app/lib`
- `apps/app/lib/api.ts` - mock async API layer; mirrors the real `/v2` client surface.
- `apps/app/lib/api-client.ts` - actual HTTP client wrapper for `/v2` requests.
- `apps/app/lib/mock-data.ts` - seed/mock contract types and records.
- `apps/app/lib/format.ts` - shared display-format helpers.
- `apps/app/lib/pagination.ts` - UI pagination helper for page-number chips.
- `apps/app/lib/utils.ts` - `cn()` class merge helper.
- `apps/app/lib/analytics/types.ts` - analytics DTOs and aggregation shapes.
- `apps/app/lib/analytics/aggregate.ts` - analytics reducer used by the dashboard.
- `apps/app/lib/analytics/mock-timeseries.ts` - analytics time-series fixtures.
- `apps/app/lib/analytics/range.ts` - date-range helpers.
- `apps/app/lib/widgets/widget-types.ts` - widget studio contract types.
- `apps/app/lib/widgets/widget-presets.ts` - widget preset defaults and builders.
- `apps/app/lib/widgets/widget-fallback-testimonials.ts` - preview-only fallback testimonial pool.
- `apps/app/lib/widgets/widget-studio-store.ts` - widget studio local persistence and mutation store.
- `apps/app/lib/widgets/widget-token-css.ts` - widget design-token-to-CSS mapping.
- `apps/app/lib/collect/types.ts` - canonical testimonial form config types and migration helpers.
- `apps/app/lib/collect/studio-types.ts` - form studio question/layout/design types.
- `apps/app/lib/collect/presets.ts` - form preset definitions.
- `apps/app/lib/collect/studio-presets.ts` - form studio presets and default builder.
- `apps/app/lib/collect/studio-store.ts` - form studio local persistence and mutation store.
- `apps/app/lib/collect/studio-token-css.ts` - form design-token-to-CSS mapping.

### Exported `api*` functions
- `apps/app/lib/api-client.ts`: `export async function apiRequest<T>(path: string, token: string | null, options: RequestInit = {}): Promise<T>`.
- `apps/app/lib/api.ts`: `export async function apiGetProjects(): Promise<MockProject[]>`.
- `apps/app/lib/api.ts`: `export async function apiGetProject(slug: string): Promise<MockProject | null>`.
- `apps/app/lib/api.ts`: `export async function apiGetTestimonials(projectId: string, filter: TestimonialsFilter = {}): Promise<PaginatedResponse<MockTestimonial>>`.
- `apps/app/lib/api.ts`: `export async function apiApproveTestimonial(id: string): Promise<{ success: boolean }>`.
- `apps/app/lib/api.ts`: `export async function apiRejectTestimonial(id: string): Promise<{ success: boolean }>`.
- `apps/app/lib/api.ts`: `export async function apiPublishTestimonial(id: string, published: boolean): Promise<{ success: boolean }>`.
- `apps/app/lib/api.ts`: `export async function apiGetTestimonial(projectId: string, testimonialId: string): Promise<MockTestimonial | null>`.
- `apps/app/lib/api.ts`: `export async function apiGetWidgets(projectId: string): Promise<MockWidget[]>`.
- `apps/app/lib/api.ts`: `export async function apiGetApiKeys(projectId: string): Promise<MockApiKey[]>`.
- `apps/app/lib/api.ts`: `export async function apiGetApiKeyById(keyId: string): Promise<MockApiKey | null>`.
- `apps/app/lib/api.ts`: `export async function apiGetApiKeyEvents(keyId: string): Promise<MockApiKeyEvent[]>`.
- `apps/app/lib/api.ts`: `export async function apiCreateApiKey(projectId: string, draft: CreateApiKeyDraft): Promise<{ key: MockApiKey; plaintext: string }>`.
- `apps/app/lib/api.ts`: `export async function apiRevokeApiKey(keyId: string): Promise<{ success: boolean }>`.
- `apps/app/lib/api.ts`: `export async function apiRotateApiKey(keyId: string): Promise<{ plaintext: string }>`.
- `apps/app/lib/api.ts`: `export async function apiUpdateApiKey(keyId: string, patch: ApiKeyPatch): Promise<MockApiKey>`.
- `apps/app/lib/api.ts`: `export async function apiUpdateProject(slug: string, patch: ProjectPatch): Promise<MockProject>`.
- `apps/app/lib/api.ts`: `export async function apiGetNotifications(): Promise<MockNotification[]>`.
- `apps/app/lib/api.ts`: `export async function apiGetSubscription(): Promise<MockSubscription>`.
- `apps/app/lib/api.ts`: `export async function apiCancelSubscription(): Promise<MockSubscription>`.
- `apps/app/lib/api.ts`: `export async function apiSwitchPlan(planId: string): Promise<MockSubscription>`.
- `apps/app/lib/api.ts`: `export async function apiGetInvoices(): Promise<Invoice[]>`.
- `apps/app/lib/api.ts`: `export async function apiGetPaymentMethods(): Promise<PaymentMethod[]>`.
- `apps/app/lib/api.ts`: `export async function apiDeletePaymentMethod(id: string): Promise<void>`.
- `apps/app/lib/api.ts`: `export async function apiSetDefaultPaymentMethod(id: string): Promise<PaymentMethod[]>`.
- `apps/app/lib/api.ts`: `export async function apiGetBillingProfile(): Promise<BillingProfile>`.
- `apps/app/lib/api.ts`: `export async function apiUpdateBillingProfile(input: Partial<BillingProfile>): Promise<BillingProfile>`.
- `apps/app/lib/api.ts`: `export async function apiGetUser(): Promise<MockUser>`.
...
### Webhooks
- Domain summary: Webhooks are the external ingress path for identity and payments. In api the only implemented webhook is Clerk user sync; the legacy app also has Razorpay webhook processing.
...
- Web_v2 client calls: none.
- Mock data shape: no alert-specific mock layer in `app`.
...
- Web_v2 client calls: none.
- Mock data shape: no admin mock layer in `app`.
- Request DTOs: none in current UI contract.
- Response shapes: none in current UI contract.
...
- Pagination: `apps/api` does not have source `base-v2` files in the scaffold, but the generated declarations and graph report show a `BaseV2Service` + `paginate()` + `pagination()` pair. The visible contract is `V2PaginatedResponse` with `items`, `total`, `page`, `pageSize`, `totalPages`, `hasNext`, and `hasPrev`; the legacy Express helper uses `ResponseHandler.paginated()` with `meta.pagination { page, limit, total, totalPages, hasNextPage, hasPreviousPage }`.
...
- Serializer pattern: `apps/api/src/modules/base-v2/` is missing from the source tree, so there is no concrete serializer implementation to inspect; the generated declarations show DTO-returning service methods, which implies direct DTO serialization rather than Express-style envelopes.
- Endpoint prefixing: `apps/api/src/main.ts` applies the global `/v2` prefix to everything except `health`, so the dossier should treat all API routes as `/v2/...` paths.
- HTTP client: `apiRequest()` in `apps/app/lib/api-client.ts` always prefixes with `/v2` and throws `ApiError(status, text)` on non-2xx responses, so response bodies should be human-readable on failure.
...
- Scope revision signal: alerts and ops/admin currently have no `app` client calls, so if the orchestrator wants them in v2 this pass should be treated as schema and backend groundwork only, not a completed UI contract.
