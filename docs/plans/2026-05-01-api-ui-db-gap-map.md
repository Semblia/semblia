...
| Users and onboarding | Welcome flow is dark and uses `localStorage("tresta:onboarding:done")`; profile/security mostly use Clerk. | `GET /v2/me`, `PATCH /v2/me`, `POST /v2/me/onboarding/complete`. | `User.onboardingCompletedAt` exists. | P0 for onboarding; no server-backed redirect gate in web. Optional onboarding answers have nowhere to live. | Wire current user and onboarding completion. If job title/referral/intent matter, add `UserOnboardingProfile` or explicit nullable fields, not ad hoc Clerk metadata. |
...
5. Which app handles wildcard/default hosted domains for `<project-slug>.testimonials.tresta.app`, forms, and walls?
