...
| LinkedIn                | Enable Clerk LinkedIn OIDC; configure app/callback URLs; request only approved products/scopes.         | Baseline OIDC is `openid`, `profile`. This import also requires LinkedIn-restricted `r_member_social` to retrieve the connected member's posts; it remains unavailable until that product/scope is approved and configured. Never claim recommendation or arbitrary profile scraping. |
